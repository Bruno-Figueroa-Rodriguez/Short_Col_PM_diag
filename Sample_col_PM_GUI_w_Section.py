...
class MyWindow(QWidget):
    def __init__(self):
        ...
        self.button.clicked.connect(self.plot_section)
        ...
        self.section_figure = plt.Figure()
        self.section_canvas = FigureCanvas(self.section_figure)
        self.layout.addWidget(self.section_canvas)     
        self.ax2 = self.section_figure.add_subplot(211)
    ...
    def generate_and_plot(self):
        ...
        self.ax1.grid()
        self.ax1.set(xlabel='Moment (Kip-ft)', ylabel='Axial Loading (Kip)', title='ACI 318 - PM Interaction Diagram')
        self.canvas.draw()

        #self.plot_section(b, h, num_lay, bar_per_lay, bar_size, CC_to_CL_long)

#    def plot_section(self, b, h, num_lay, bar_per_lay, bar_size, CC_to_CL_long):
    def plot_section(self):
        ...
        self.ax2.add_patch(plt.Rectangle((-b/2, -h/2), b, h, fill=False, edgecolor='black', linewidth=2))
        rebar_spacing = (h - 2*CC_to_CL_long) / (num_lay - 1)
        for i in range(num_lay):
            y_coord = -h/2 + CC_to_CL_long + i * rebar_spacing
            for j in range(bar_per_lay):
                x_coord = -b/2 + CC_to_CL_long + j * (b - 2*CC_to_CL_long) / (bar_per_lay - 1)
                self.ax2.add_patch(plt.Circle((x_coord, y_coord), rebars[bar_size]['diam']/2, color='red'))
                #self.ax2.add_patch(plt.Circle((x_coord, y_coord), radius=int(bar_size[1:])/8/2, color='red'))
        
        self.ax2.set_xlim(-b/2 - 2, b/2 + 2)
        self.ax2.set_ylim(-h/2 - 2, h/2 + 2)
        self.ax2.set_aspect('equal')
        self.ax2.set_xlabel("Width (in)")
        self.ax2.set_ylabel("Height (in)")
        self.ax2.set_title("Column Section")
        self.ax2.grid()
